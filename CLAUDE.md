...
4. CI reads `.nvmrc` via `actions/setup-node`'s `node-version-file`, so no workflow edits are needed. (This repo currently uses Travis CI with no `actions/setup-node` step; if you migrate to GitHub Actions, add one pointing at `node-version-file: '.nvmrc'`.)
