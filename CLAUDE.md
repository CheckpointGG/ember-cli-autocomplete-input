...
4. CI reads `.nvmrc` via `actions/setup-node`'s `node-version-file`, so no workflow edits are needed. (This repo has no CI: it carries an inherited upstream `.travis.yml` that nothing runs, has no GitHub Actions workflow at any ref, and has never had a run. If you add GitHub Actions, include an `actions/setup-node` step pointing at `node-version-file: '.nvmrc'`.)
